...
async function start() {
  ...
  run(`CREATE TABLE IF NOT EXISTS tasks (
    ...
    location TEXT, deadline TEXT, completed_at TEXT,
  ...
  if (get('SELECT COUNT(*) as c FROM users').c == 0) {
    ...
    [
      ['Admin','admin@geo.uz',h('1234'),'admin','Direktor','Boshqaruv'],
      ['Nodir Nazoratchi','nazoratchi@geo.uz',h('1234'),'supervisor','Rahbar','Guruh A'],
  ...
  app.post('/api/tasks', auth, adminOrSup, (req, res) => {
    const { title, description, assigned_to, priority, location, deadline } = req.body;
    if (!title) return res.status(400).json({ error: 'Title required' });
    run('INSERT INTO tasks (title,description,assigned_to,created_by,priority,location,deadline) VALUES (?,?,?,?,?,?,?)',
      [title, description||'', assigned_to||null, req.user.id, priority||'medium', location||'', deadline||null]);
    res.json({ message: 'Created' });
  });
