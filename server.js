...
function run(sql, p=[]) {
  try { execute(sql, p); return { ok: true }; } catch(e) { console.error('SQL error:', e.message); return { ok: false, error: e.message }; }
}
function all(sql, p=[]) {
  try {
...
app.post('/api/auth/register', requireDb, (req, res) => {
  try {
    const {
      name, surname, firstname, middlename, jshshir, passport_id, face_photo,
      email, password, group_name, position, phone
    } = req.body;

    if (!name || !email || !password) {
      return res.status(400).json({ error: 'Ism, email va parol kerak' });
    }
    ...
    execute(
      `INSERT INTO users
       (name,surname,firstname,middlename,jshshir,passport_id,face_photo,email,password,role,position,group_name,phone,status,created_at)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
        name, surname||'', firstname||'', middlename||'', jshshir||'', passport_id||'', face_photo||'',
        email, bcrypt.hashSync(password,10), 'worker', position||'', group_name||'', phone||'', 'pending', nowUZ()
      ]
    );
...
app.get('/api/users', requireDb, auth, adminOrSup, (req, res) => {
  if (req.user.role === 'admin')
    return res.json(all('SELECT id,name,surname,firstname,middlename,jshshir,passport_id,face_photo,email,role,position,group_name,phone,status FROM users ORDER BY role,name'));
  res.json(all('SELECT id,name,surname,firstname,middlename,jshshir,passport_id,face_photo,email,role,position,group_name,phone,status FROM users WHERE group_name=? ORDER BY name', [req.user.group]));
});

app.post('/api/users', requireDb, auth, adminOnly, (req, res) => {
  const { name, surname, firstname, middlename, jshshir, passport_id, face_photo, email, password, role, position, group_name, phone } = req.body;
  if (!name || !email || !password) return res.status(400).json({ error: 'Ism, email, parol kerak' });
  try {
    execute(
      `INSERT INTO users
       (name,surname,firstname,middlename,jshshir,passport_id,face_photo,email,password,role,position,group_name,phone,created_at)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
        name, surname||'', firstname||'', middlename||'', jshshir||'', passport_id||'', face_photo||'',
        email, bcrypt.hashSync(password,10), role||'worker', position||'', group_name||'', phone||'', nowUZ()
      ]
    );
...
app.put('/api/users/:id', requireDb, auth, adminOnly, (req, res) => {
  const { name, surname, firstname, middlename, jshshir, passport_id, face_photo, position, group_name, phone, role, status, password } = req.body;
  ...
  execute(
    `UPDATE users SET
     name=?,surname=?,firstname=?,middlename=?,jshshir=?,passport_id=?,face_photo=?,
     position=?,group_name=?,phone=?,role=?,status=?,password=? WHERE id=?`,
    [
      name||t.name, surname??t.surname, firstname??t.firstname, middlename??t.middlename,
      jshshir??t.jshshir, passport_id??t.passport_id, face_photo??t.face_photo,
      position||t.position, group_name||t.group_name, phone||t.phone,
...
async function initDB() {
  ...
  try {
    ...
    addColumnIfMissing('users', 'surname', 'TEXT DEFAULT ""');
    addColumnIfMissing('users', 'firstname', 'TEXT DEFAULT ""');
    addColumnIfMissing('users', 'middlename', 'TEXT DEFAULT ""');
    addColumnIfMissing('users', 'jshshir', 'TEXT DEFAULT ""');
    addColumnIfMissing('users', 'passport_id', 'TEXT DEFAULT ""');
    addColumnIfMissing('users', 'face_photo', 'TEXT DEFAULT ""');
